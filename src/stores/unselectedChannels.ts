...
const unselectedChannels = createSlice({
    ...
    reducers: {
        ...
        deleteUnselectedChannel: (state, action) => {
            state.unselectedChannels = state.unselectedChannels.filter((channel: IChannel) => channel.id !== action.payload)
        }
    }
})

export const { setUnselectedChannels, addUnselectedChannel, deleteUnselectedChannel } = unselectedChannels.actions
